...
class pfcGlobalConfig
{
     ...
     */
    public $refresh_delay_steps = array(2000, 20000, 3000, 30000, 5000, 60000, 8000, 300000, 15000, 600000, 30000);
     ...
     */
    public $lockurl = 'http://www.phpfreechat.net';
     ...
     * Default value: 150)</p>.
     */
    public $max_displayed_lines = 150;
     ...
     * (Default value: false)</p>.
     */
    public $quit_on_closedwindow = true;
     ...
     */
    public $connect_at_startup = true;

    /**
     * <p>Setting it to true will start the chat minimized.
     * (Default value: false)</p>.
     */
    public $start_minimized = false;
     ...
     */
    public $bbcode_colorlist = array('#FFFFFF',
                                '#000000',
                                '#000055',
                                '#008000',
                                '#FF0000',
     ...
     */
    public $nickname_colorlist = array('#CCCCCC',
                                  '#000000',
                                  '#3636B2',
                                  '#2A8C2A',
                                  '#C33B3B',
     ...
     * (Default value: false)</p>.
     */
    public $get_ip_from_xforwardedfor = false;
    ...
    public $errors = array();
    public $is_init = false; // used internaly to know if the chat config is initialized
  public $version = ''; // the phpfreechat version: taken from the 'version' file content

  public $_sys_proxies = array('lock', 'checktimeout', 'checknickchange', 'auth', 'noflood', 'censor', 'log');
    public $_dyn_params = array('nick', 'isadmin', 'islocked', 'admins', 'frozen_channels', 'channels', 'privmsg', 'nickmeta', 'time_offset', 'date_format', 'time_format');
    public $_params_type = array();
    public $_query_string = '';

    public function pfcGlobalConfig($params = array())
    {
    ...
    public static function &Instance($params = array(), $destroy_instance = false)
    {
        static $i;
        if ($destroy_instance) {
            $i = null;
     ...
     */
    public function _saveParamsTypes()
    {
    ...
    public function loadSmileyTheme()
    {
        ...
        foreach ($theme as $line) {
            ...
            if (preg_match('/^#.*/', $line)) {
                continue;
            } elseif (preg_match("/([a-z_\-0-9\.]+)(.*)$/i", $line, $res)) {
    ...
    public function _GetCacheFile($serverid = '', $data_private_path = '')
    {
        if ($serverid == '') {
            $serverid = $this->getId();
        }
     ...
     */
    public function synchronizeWithCache()
    {
        $cachefile = $this->_GetCacheFile();
        $cachefile_lock = $cachefile.'_lock';
        ...
        if (file_exists($cachefile)) {
            ...
            if (file_exists($cachefile_lock)) {
                ...
                if ($locktime + 15 < time()) {
                    ...
                    return false;
                } // do nothing if the lock file exists
            } else {
                @touch($cachefile_lock);
            } // create the lockfile
            ...
            if (count($errors) == 0) {
                ...
                @unlink($cachefile_lock);
            } // destroy the lock file for the next attempt
    ...
    public function isDefaultFile($file)
    {
    ...
    public function getFilePathFromTheme($file)
    {
        if (file_exists($this->theme_path.'/'.$this->theme.'/'.$file)) {
            return $this->theme_path.'/'.$this->theme.'/'.$file;
        } elseif (file_exists($this->theme_default_path.'/default/'.$file)) {
    ...
    public function getFileUrlFromTheme($file)
    {
        if (file_exists($this->theme_path.'/'.$this->theme.'/'.$file)) {
