...
$(document).ready(function(){
	...
	var dialoglinks = $('a[href^="#openModal"]');
	var closelinks = $('.close');
	var dialogs = $('[id^="openModal"]');

	dialoglinks.click( function (event) {
		event.preventDefault();
		var scen = $(this).attr("href");
		$(scen).css({ 'opacity' : 1.0 });
		$(scen).css({ 'pointer-events' : 'auto' });
		return false;
	});
	closelinks.click( function (event) {
		event.preventDefault();
		$(dialogs).css({ 'opacity' : 0.0 });
		$(dialogs).css({ 'pointer-events' : 'none' });
		return false;
	});
