...
<html lang="<?php echo $PHORUM['locale']; ?>">
  ...
  <body onload="{IF FOCUS_TO_ID}var focuselt=document.getElementById('{FOCUS_TO_ID}'); if (focuselt) focuselt.focus();{/IF}">
    <div align="{forumalign}">
      <div class="PDDiv">
		<div style="height:50px; overflow:hidden; box-shadow: 0px 2px 8px -4px #333333; -webkit-box-shadow: 0px 2px 8px -4px #333333;width:100%;">
      {IF NOT PRINTVIEW}
        ...
        {IF URL->INDEX}
		<a href="{URL->INDEX}">{/IF}<img src="templates/classic/images/logo.png" alt="EMRE Forum" title="EMRE Forum" height="36px;"  border="0" />{IF URL->INDEX}</a>{/IF}
